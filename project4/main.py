...
account_pin_label = tk.Label(win, text="Account Number/Pin Number")
...
# Pin number label
acn_label = tk.Label(win, text="Account Number", width=30)
...
def handle_pin_button(event):
    ...
    if len(pin_number) == 4:
        tk.messagebox.showerror(title="Error", message="please enter PIN with 4 digits")
        return
    # Set the new pin number on the pin_number_var
    pin_number = pin_number_var.get()
    pin_number = pin_number + str(event.widget["text"])
...
def log_in(event):
    ...
    if len(acn) == 0:
        ...
        account.reset()
        tk.messagebox.showerror(title="Error", message="enter your account number.")
        return
    pin_number = pin_number_var.get()
    if len(pin_number) != 4:
        tk.messagebox.showerror(title="Error", message="please enter PIN with 4 digits")
    ...
    if os.path.isfile(file_name):
        ...
        if len(lines) < 6:
            account.reset()
            tk.messagebox.showerror(title="Error", message="Error while processing your request. Try again later")
            return
        # First line is account number
        # Second line is PIN number, raise exception if the PIN entered doesn't match account PIN read
        if pin_number != lines[1]:
            # Reset account object
            account.reset()
            tk.messagebox.showerror(title="Error", message="Invalid PIN")
            return

        # Read third and fourth lines (balance and interest rate)
        account.balance = float(lines[2])
        account.interest_rate = float(lines[3])
        ...
        for i in range(4, len(lines), 2):
            transaction_tuple = (lines[i], lines[i + 1])
        ...
        account.reset()
        tk.messagebox.showerror(title="Error", message="account number not found.")
        return
...
def create_login_screen():
    ...
    account_pin_label.grid(column=0, row=1, sticky=N + S + W + E)
